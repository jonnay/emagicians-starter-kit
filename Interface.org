...
#+end_src
   ...
   I am up in the air about this one
...
Serious magick to force the user to use 
delete me soon
#+begin_src emacs-lisp :tangle no
(defun emagician/make-muscle-memory (the-problem the-solution)
  "The user creation funtion
  ...
  (let* ((required-corrections 3)
    ...
    (while (< attempts required-corrections)
      ...
      (pcase (read-key-sequence
        ...
        ((pred (equal (kbd "C-g")))
         (setq attemps required-corrections)
...
(eval-after-load 'helm-command
  '(add-function :after (symbol-function 'helm-M-x) #'emagician/helm-M-x/suggest-keybinding))
